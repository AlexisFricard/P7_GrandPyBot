...
def test_fct_get_datas_return_expected_datas():
    ...
    assert mock_response[0]["address"] == (
        "18 Rue Alphonse Caillaud, 49400 Saumur, France"
    )
...
def test_fct_get_datas_return_error_msg():
    ...
    assert mock_response[0] is None
    assert mock_response[1] == (
        "Désolé 🤷‍♂️ Mais je n'ai pas compris votre question ! 😇"
    )
...
def test_fct_get_response_return_json_str():
    ...
    # JSON DUMP
    assert type(mock_response) is str
